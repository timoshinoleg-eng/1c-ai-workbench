...
def main():
    ...
    if not args.NoValidate:
        ...
        if os.path.exists(validate_script):
            ...
            # SECURITY: shell=False blocks command injection via target_xml.
            # Historically this used os.system with an f-string interpolated
            # command line; a " in the subsystem name broke out of the quoted
            # -SubsystemPath argument and ran arbitrary commands. The vendored
            # upstream carried the unsafe call until commit fixing it in
            # 1c-ai-workbench; the same fix is being sent to the upstream repo
            # so that future 'git subtree pull' merges do not reintroduce it.
            subprocess.run(
                [
                    "powershell.exe",
                    "-NoProfile",
